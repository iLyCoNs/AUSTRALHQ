...
def build_pdf(filename):
    ...
    cover_title_style = ParagraphStyle(
        ...
        fontName='Helvetica-Bold', fontSize=20, leading=24, textColor=colors.white
    )
    cover_sub_style = ParagraphStyle(
        'CoverSub', parent=styles['Normal'],
        fontName='Helvetica', fontSize=10, leading=14, textColor=colors.HexColor('#cbd5e1')
    )
    ...
    h2_style = ParagraphStyle(
        ...
        fontName='Helvetica-Bold', fontSize=10, leading=13, textColor=COLOR_PRIMARY, spaceAfter=3
    )
    body_style = ParagraphStyle(
        'Body', parent=styles['Normal'],
        fontName='Helvetica', fontSize=8.2, leading=11.8, textColor=COLOR_TEXT, spaceAfter=4
    )
    ...
    header_table = Table(header_data, colWidths=[548])
    ...
    t_bench = Table(bench_data, colWidths=[120, 110, 150, 168])
    ...
    # =============================================================
    # SECCIÓN 2: DIAGNÓSTICO DE LA REGIÓN DE LOS LAGOS
    # =============================================================
    ...
    t_steps = Table(steps_data, colWidths=[45, 235, 268])
    ...
    p_db = ("La versión v4.1 incorpora un **Motor de Base de Datos Relacional SQL (CamilaDB)** compatible con **SQLite Local ($0 USD)** y **Turso Cloud SQLite (9 GB gratis en la nube)**, estructurando 4 tablas clave:")
    ...
    t_symbiosis = Table(symbiosis_data, colWidths=[130, 205, 213])
    ...
    comp_data = [
        [Paragraph("Característica", table_header_style), Paragraph("Chatbot Tradicional / WhatsApp Bot Rígido", table_header_style), Paragraph("Secretaría Camila™ IA 24/7 v4.1", table_header_style)],
        [
    ...
    t_comp = Table(comp_data, colWidths=[130, 205, 213])
    ...
    t_planes = Table(planes_data, colWidths=[150, 130, 268])
