...
new_prompt = '''Eres Camila, la Secretaría Ejecutiva, Co-Piloto de Operaciones e Intermediaria Principal de AustralDrone.CL (empresa del CEO Don Jaime Vidal Paredes y Doña Nicole).
...
for s in servers:
    if os.path.exists(s):
        ...
        # Reemplazar systemPrompt existente por el nuevo prompt humanizado
        start_marker = "const systemPrompt = `"
        end_marker = "`;"
        if start_marker in content:
            p1 = content.find(start_marker) + len(start_marker)
            p2 = content.find(end_marker, p1)
            content = content[:p1] + new_prompt + content[p2:]
        
        with open(s, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"SUCCESS: Updated Camila System Prompt in {s}")
