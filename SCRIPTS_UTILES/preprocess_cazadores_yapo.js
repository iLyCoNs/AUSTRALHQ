...
// Lista de nuevas skins a procesar: [srcFileName, outNormName]
const ITEM_LIST=[
  ['CAZADOR360', 'CAZADOR360'],
  ['CAZADORVENTA', 'CAZADORVENTAS'],
  ['YAPOCL', 'YAPOCL']
];

(async()=>{
  console.log('=== GENERANDO _norm.png PARA CAZADOR360, CAZADORVENTA Y YAPOCL ===');
  const b=await chromium.launch({channel:'msedge',headless:true});
  const page=await b.newPage();

  for(const [srcName, outName] of ITEM_LIST){
    console.log(`\nProcesando ${srcName}.png -> ${outName}_norm.png...`);
    const result=await page.evaluate(async([srcN, targetFw, targetFh, thresh])=>{
      const img=new Image();
      img.crossOrigin='anonymous';
      await new Promise((res,rej)=>{
        img.onload=res;
        img.onerror=()=>rej(new Error('No carga sprite: '+srcN));
        img.src='http://localhost:8080/sprites/'+srcN+'.png';
      });
      ...
      // Convertir fondo blanco/claro a transparente
      ...
      // BFS Blob Detection para aislar cada personaje por frame
      ...
      // Ordenar blobs por filas de ~288px
      const rows=[[], [], [], []];
      blobs.forEach(b=>{
      ...
      // Crear nuevo canvas 12cols x 4rows normalizado
    ...
    }, [srcName, TARGET_FW, TARGET_FH, THRESHOLD]);
  ...
  await b.close();
  console.log('\n✓ CAZADOR360_norm.png + CAZADORVENTAS_norm.png + YAPOCL_norm.png GENERADOS CORRECTAMENTE!');
