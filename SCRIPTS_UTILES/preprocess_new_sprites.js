...
// Lista de todas las skins PNG para procesar al formato _norm.png
const SPRITES=['DIEGO', 'CABALLOTROYA', 'YAPOCL', 'CAZADOR360', 'CAZADORVENTAS'];

(async()=>{
  console.log('=== PREPROCESANDO TODAS LAS NUEVAS SKINS A FORMATO _norm.png ===');
  ...
  for(const name of SPRITES){
    ...
    const result=await page.evaluate(async([n, targetFw, targetFh, thresh])=>{
      ...
      // BFS Blob Detection para aislar cada personaje
      ...
      // Crear nuevo canvas grid perfecto de 12cols x 4rows
    ...
    console.log('✓ Guardado sprite normalizado: '+outPath+' ('+Math.round(buf.length/1024)+'KB)');
  }

  await b.close();
  console.log('\n✓ TODAS LAS SKINS CONVERTIDAS Y NORMALIZADAS CORRECTAMENTE!');
